...
// Visualização
void mostramemóriaPricipal(t_celula MemPrincipal[])
{
	TELA;
...
void mostramemóriaCache(t_quadro MemCache[])
{
	TELA;
	printf("\nMostrando MemCache: \n");
	printf("Alterado");
	...
	for (int mostratodoCache = 0; mostratodoCache < 64;)
	{
		for (int i = 0; i < 16; i++)
		{
			...
			for (int h = 0; h < TAMST; h++)
			{
				printf("%d", MemCache[i].bloco_cache.celula_bloco[0].endereco[h]);
			}
		}
	}
...
int main(void)
{
	...
	for (int j = 0; j < 2048;)
	{
		// inserindo conteudo em quatro células de memória
		for (int i = 0; i < 4; i++)
		{
	...
	while (1)
	{
		...
		case 3:
			break;
		case 4:
			mostramemóriaPricipal(MemPrincipal);
			break;
		case 5:
			mostramemóriaCache(MemCache);
			break;
		default:
			printf("\nOpcao incorreta");
			getchar();
